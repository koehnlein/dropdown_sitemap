	...
	class tx_dropdownsitemap_pi1 extends tslib_pibase {
		...
		function main($content,$conf) {
			...
			if ($this->conf['expAllLink'] == 1) {
				$content[] = '<div class="expAll"><a href="javascript:' . $this->prefixId . '_expAll();">' . $this->pi_getLL('expall') . '</a></div>';
			}
		...
		function buildImageTSConfig($expanded=false) {
			...
			// HTML tag parameters
			$imgTSConfigSub['params'] = $this->conf['picture.']['params'] . ' id="pic_{elementUid}"';
		...
		function buildJSCode() {
			...
			// Plus image URL
			$plusImgURL = str_replace(PATH_site,'',t3lib_div::getFileAbsFileName($this->conf['picture.']['expand']));
			
			// Minus image URL
			$minusImgURL = str_replace(PATH_site,'',t3lib_div::getFileAbsFileName($this->conf['picture.']['collapse']));
